...
void playPvAI(){
  ...
  if(millis()-lastMove>50){
    ...
    int targetY = ballY + random(-errorRange, errorRange+1);

    if(targetY < p2Y + paddleH/2) p2Y = max(p2Y-2, 12);
    else if(targetY > p2Y + paddleH/2) p2Y = min(p2Y+2, 64-paddleH);
    lastMove=millis();
  }
...
void playAIvsAI(){
  ...
  if(millis()-lastMove1>60){
    ...
    int target1 = ballY + random(-err1, err1+1);

    if(target1 < p1Y + paddleH/2) p1Y = max(p1Y-2, 12);
    else if(target1 > p1Y + paddleH/2) p1Y = min(p1Y+2, 64-paddleH);
    lastMove1=millis();
  }
  if(millis()-lastMove2>60){
    int err2 = map(100 - ai2Accuracy, 0, 100, 0, 12);
    int target2 = ballY + random(-err2, err2+1);

    if(target2 < p2Y + paddleH/2) p2Y = max(p2Y-2, 12);
    else if(target2 > p2Y + paddleH/2) p2Y = min(p2Y+2, 64-paddleH);
    lastMove2=millis();
  }
  clampPaddles(); updateBall();
}
...
static uint32_t debounce1=0, debounce2=0, debounce3=0;
bool readButtonOnce(uint8_t pin){
  uint32_t now = millis();
  bool pressed = (digitalRead(pin)==LOW);
  uint32_t* tp = (pin==BTN1? &debounce1 : (pin==BTN2? &debounce2 : &debounce3));
